//! HistoryEntry 数据边界：描述一次可撤销纯文本历史节点所需的 undo/redo 批次。
//!
...
use crate::{TransactionId, transaction::EditList};
...
impl HistoryEntry {
    ...
    /// 字节和；description / TextRange / EditList 容器本身不计入。
    /// 这反映了 Undo 复原所需字符串的实际开销，是文本内核能稳定承诺的最小事实。
