//! M6C GPUI testbed：IME / composition 组合输入语义。
//!
//! 这个 example 是“人类体感 / UI 桥接”验证入口，不替代 `tests/m6c_composition.rs`。
//! 当前 GPUI 示例先用快捷键模拟系统 IME 事件：start / update / commit / cancel，
//! 重点验证 M6C public API 与 UI 状态同步，并继承 M6B 的多光标、移动、Undo / Redo 等手感。
...
const SAMPLE_TEXT: &str = "M6C IME composition 体验台\n\n\
英文区域：hello world\n\
中文输入区域：|\n\
日文输入区域：|\n\
韩文输入区域：|\n\
...
impl M6cTestbed {
    fn new(cx: &mut Context<Self>) -> Self {
        let mut buffer = Buffer::from_text(SAMPLE_TEXT.to_string(), BufferConfig::default())
            .expect("M6C testbed sample text should be valid");
        buffer.mark_saved();
        ...
        let mut this = Self {
            ...
            message: "M6C 已就绪：composition start / update / commit / cancel".to_string(),
    ...
    fn status_lines(&self) -> Vec<String> {
        ...
        vec![
            format!(
                "M6C GPUI testbed | {} | dirty={} | v{} | 保存点={} | 行数={} chars={} bytes={}",
    ...
    fn help_lines(&self) -> Vec<&'static str> {
        vec![
            ...
            "M6C composition：Cmd-I start，Cmd-K update raw 'n'，Cmd-L update '你'，Cmd-Y update '你好'",
...
fn main() {
    Application::new().run(|cx: &mut App| {
        ...
        let window = cx
            ...
            .expect("open M6C testbed window");
        ...
        window
            ...
            .expect("focus M6C testbed window");
    });
}
