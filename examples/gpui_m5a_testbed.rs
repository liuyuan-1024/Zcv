...
use zom_engine::{
    ...
    LogicalColumn, Position, SelectionSnapshot, Snapshot, TextRange, Transaction, Utf16Position,
...
const INITIAL_TEXT: &str = concat!(
    ...
    "M5A 额外显示 CharOffset、ByteOffset、UTF-16 Position、byte↔UTF-16 roundtrip、grapheme boundary 和文件换行风格。\n",
    "M5A 左右移动与 Backspace/Delete 使用 grapheme-safe boundary，避免切开 ZWJ emoji 与组合字符。\n",
...
impl M5ATestbed {
    ...
    fn coordinate_summary_label(&self) -> String {
        match self.current_coordinate_probe() {
            Ok(probe) => format!(
                "byte={} utf16=({}, {}) grapheme={} prev_g={} next_g={}",
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .mb_4()
                    .child(format!(
                        "Zom Engine M5A | char={} / {} | line={} col={} | version={} saved={} | dirty={} | A={} | B={} | changed_ranges={} | {} | {} | {} | {} | {}",
                        cursor.get(),
                        self.buffer.len_chars().get(),
                        position.line().get(),
                        position.column().get(),
            ...
            .child(
                div()
                    ...
                    .child("输入字符 / Space / Tab / Enter；连续输入会合并为一个 Undo；Backspace / Delete；← →；Home / End；Cmd-S 保存；Cmd-R 重置；Cmd-B 批量事务；Cmd-M 合并事务；Cmd-Z/Ctrl-Z 撤销；Cmd-Shift-Z/Ctrl-Y 重做；Cmd-K 捕获 Snapshot；Cmd-L 插入长行 smoke；Cmd-U 插入 Unicode/CRLF 探针；←/→/Backspace/Delete 使用 grapheme-safe boundary"),
            )
