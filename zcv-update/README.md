...
- 启动确认失败并回滚后，应用下次启动读取 `last-result.json`，以一次性 Toast 告知失败原因；已应用的更新只记录并从文件系统消费该结果，不打扰用户。
- `zcv-update` 负责纯协议与文件系统操作，不能反向依赖应用 UI。
- helper 只消费应用进程已经验证并写入的事务，不下载更新，也不选择版本。
- 事务状态机、启动确认与结果落盘是 `zcv-update` 的平台无关编排；`zcv-update-helper` 与 `src/platform/` 后端只提供进程退出等待、路径布局和原子切换/回滚原语。
...
- Windows 更新在应用退出后由 helper 请求释放外部文件占用，备份旧目录并切换新目录；启动确认失败时恢复旧目录。
