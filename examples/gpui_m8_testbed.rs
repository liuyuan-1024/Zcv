//! M8 GPUI testbed：PositionMap、DeltaEvent 与 M7 既有编辑体验。
//!
//! 这个 example 是“人类体感 / UI 桥接”验证入口，不替代 `tests/m8_position_map.rs`。
//! 它继承 M7 的输入、移动、多光标、Undo / Redo、composition、reload 与保存边界体感，
//! 并叠加 PositionMap、DeltaEvent、pending event queue 和 changed ranges 可视化。
...
const SAMPLE_TEXT: &str = "M8 PositionMap / DeltaEvent 体验台\n\n\
英文区域：hello world\n\
中文输入区域：|\n\
日文输入区域：|\n\
韩文输入区域：|\n\
...
\n\
M8 状态观察：BufferId / BufferState / DeltaEvent / PositionMap / pending events。\n\
试试输入、Undo/Redo、composition、reload、清空事件队列和观察 PositionMap。";

const RELOAD_TEXT: &str = "M8 reload 后的新外部文本\n\n\
...
impl M8Testbed {
    fn new(cx: &mut Context<Self>) -> Self {
        let mut buffer = initial_buffer().expect("M8 testbed sample text should be valid");
        buffer.mark_saved();
        ...
        let mut this = Self {
            ...
            message: "M8 已就绪：PositionMap / DeltaEvent / pending events".to_string(),
    ...
    fn status_lines(&self) -> Vec<String> {
        ...
        vec![
            format!(
                "M8 GPUI testbed | {} | v{} | 保存点={} | 行数={} chars={} bytes={}",
    ...
    fn help_lines(&self) -> Vec<&'static str> {
        vec![
            ...
            "M8 事件：状态栏显示最近 DeltaEvent / PositionMap；Cmd-Shift-E 清空 pending DeltaEvent 队列",
...
impl Focusable for M8Testbed {
    fn focus_handle(&self, _: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for M8Testbed {
...
fn initial_buffer() -> EngineResult<Buffer> {
    Buffer::from_loaded_text(
        BufferKind::file("/tmp/zom-engine-m8-testbed.txt"),
    ...
    Application::new().run(|cx: &mut App| {
        ...
        let window = cx
            .open_window(
                ...
                |_, cx| cx.new(M8Testbed::new),
            )
            .expect("open M8 testbed window");
        ...
        window
            ...
            .expect("focus M8 testbed window");
    });
}
