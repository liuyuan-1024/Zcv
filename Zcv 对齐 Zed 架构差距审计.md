...
5. `GitStore` 的 hunk 暂存偏离 Zed 的乐观 index-edit 协议（原「判定不成立」的复核只检查了状态所有权，漏掉了控制流，结论有误）：旧实现把「界面线程预先算出下一份 index 全文 + 用缓存副本做一致性校验 + 校验失败就报错让用户刷新」当成写入路径，`optimistic_index_bases` 同时承担在途互斥与失败回滚，产生「变更块已过期 / 已被其他编辑改动 / 操作已失效」这类本应由 Zcv 自己重算的用户可见错误。已对齐 Zed `BufferGitState::pending_index_edits`：操作范围在当前 working 快照上解析 `Anchor`（T-3/T-8），编辑合并进该路径的乐观 index 批次，由批次基准派生完整 index 文本写盘，权威 index 文本安装新值时清除批次；`optimistic_index_bases` 与 `GitStoreEvent::IndexText` 已删除；HEAD/index 文本变化不再丢弃共享 diff，而是经 `set_base_text`/`set_index_text` 就地增量安装（§11.1）。回归见 `staging_resolves_hunk_anchors_on_the_current_working_snapshot`、`staging_two_hunks_without_waiting_merges_pending_edits`、`file_diff_is_shared_by_working_base_and_index`。
