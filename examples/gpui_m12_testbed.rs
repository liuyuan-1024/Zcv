//! M12 GPUI testbed：Search / Replace、Viewport Slicing、MetadataLayer、TrackedRange、PositionMap、DeltaEvent 与 M11 既有编辑体验。
//!
//! 这个 example 是“人类体感 / UI 桥接”验证入口，不替代 `tests/m12_search.rs`、
//! `tests/m12_replace.rs` 和 `tests/m12_regex.rs`。
//! 它继承 M11 的输入、移动、多光标、Undo / Redo、composition、reload、保存边界、
//! DeltaEvent、PositionMap、TrackedRange 与 MetadataLayer 体感，并叠加 Viewport 可见行、
//! 长行截断、大文本滚动、Snapshot 只读切片观察、当前 Buffer 查找、跳转、替换、
//! replace all 与搜索结果高亮。
...
use zom_engine::{
    ...
    Position, RegexSearchOptions, RegexSearchResult, SearchOptions, SearchResult, Selection,
...
const SAMPLE_TEXT: &str = "M12 Search / Replace / Viewport / MetadataLayer / PositionMap / DeltaEvent 体验台\n\n\
英文区域：hello world\n\
中文输入区域：| 搜索目标 搜索目标\n\
日文输入区域：|\n\
韩文输入区域：|\n\
...
\n\
搜索样本：搜索目标 SEARCH_TARGET search_target 搜索目标。\n\
正则样本：rgb(12,34,56) rgb(7,8,9) user_id=42 trace_id=1001。\n\
\n\
继承 M6 Word movement：parseHTTPResponse user_id snake_case a+b == c && value != null\n\
\n\
M12 状态观察：BufferId / BufferState / DeltaEvent / PositionMap / TrackedRange / MetadataLayer / ViewportSlice / SearchResult。\n\
试试输入、删除、查找、跳转、替换、replace all、Undo/Redo、composition、reload，并观察 search highlight、tracked range、metadata range 与 viewport 可见行读取。";

const RELOAD_TEXT: &str = "M12 reload 后的新外部文本\n\n\
reload 会重建文本存储、清空 undo/redo history、selection 回到开头，并把当前文本设为 clean。\n\
搜索目标 reload 搜索目标；正则样本：rgb(1,2,3) rgb(4,5,6)。\n\
你仍然可以继续输入、移动、多光标、搜索、替换、composition、保存或再次 reload。\n";
...
struct M12Testbed {
    ...
    regex_query: String,
    literal_replacement: String,
    regex_replacement: String,
    search_case_sensitive: bool,
    search_whole_word: bool,
    literal_search_result: Option<SearchResult>,
    regex_search_result: Option<RegexSearchResult>,
    current_search_match: Option<usize>,
    last_search_update: String,
}

impl M12Testbed {
    fn new(cx: &mut Context<Self>) -> Self {
        let mut buffer = initial_buffer().expect("M12 testbed sample text should be valid");
        buffer.mark_saved();
        ...
        let mut this = Self {
            ...
            message: "M12 已就绪：PositionMap / DeltaEvent / pending events".to_string(),
            ...
            literal_query: "搜索目标".to_string(),
            regex_query: r"rgb\((\d+),(\d+),(\d+)\)".to_string(),
            literal_replacement: "替换目标".to_string(),
            regex_replacement: "rgb[$1,$2,$3]".to_string(),
            search_case_sensitive: false,
            search_whole_word: false,
            literal_search_result: None,
            regex_search_result: None,
            current_search_match: None,
            last_search_update: "尚未运行搜索；Cmd-F literal，Cmd-Alt-F regex".to_string(),
        };
    ...
    fn reset_buffer(&mut self, cx: &mut Context<Self>) {
        match initial_buffer() {
            Ok(mut buffer) => {
                ...
                self.clear_search_state("重置后已清空搜索结果");
    ...
    fn reload_external_sample(&mut self, cx: &mut Context<Self>) {
            ...
            .is_some()
        {
            ...
            self.clear_search_state("reload 后已清空搜索结果");
            self.viewport_start_line = Line::ZERO;
    ...
    fn literal_search_options(&self) -> SearchOptions {
        SearchOptions::default()
            .with_case_sensitive(self.search_case_sensitive)
            .with_whole_word(self.search_whole_word)
    }

    fn regex_search_options(&self) -> RegexSearchOptions {
    ...
    fn active_search_len(&self) -> usize {
        match self.search_mode {
            SearchMode::Literal => self
                .literal_search_result
                .as_ref()
                .map(SearchResult::len)
                .unwrap_or(0),
            SearchMode::Regex => self
                .regex_search_result
                .as_ref()
                .map(RegexSearchResult::len)
                .unwrap_or(0),
        }
    }

    fn active_search_is_stale(&self) -> bool {
        match self.search_mode {
            SearchMode::Literal => self
                .literal_search_result
                .as_ref()
                .map(|result| result.is_stale(self.buffer.version()))
                .unwrap_or(false),
            SearchMode::Regex => self
                .regex_search_result
                .as_ref()
    ...
    fn active_search_range(&self, ordinal: usize) -> Option<TextRange> {
        match self.search_mode {
            SearchMode::Literal => self
                .literal_search_result
                .as_ref()
                .and_then(|result| result.match_at(ordinal))
    ...
    fn install_search_highlights(
        ...
        let specs = ranges.into_iter().enumerate().map(|(index, range)| {
        ...
        match self.metadata_layers.replace_layer_ranges_with_options(
            MetadataLayerKind::SearchMatch,
            self.buffer.version(),
            specs,
        ) {
            Ok(ids) => {
                self.last_metadata_update =
                    format!("SearchMatch layer 已挂载 {} 个搜索高亮", ids.len());
                true
            }
            Err(error) => {
                self.set_message(LastOperationKind::Error, format!("{error:?}"), cx);
    ...
    fn clear_search_state(&mut self, reason: &str) {
        self.literal_search_result = None;
        self.regex_search_result = None;
        self.current_search_match = None;
        self.last_search_update = reason.to_string();
    }

    fn select_search_match(&mut self, ordinal: usize, cx: &mut Context<Self>) -> bool {
        if self.active_search_is_stale() {
            self.set_message(
                LastOperationKind::Error,
                "搜索结果已过期：请重新运行 literal 或 regex 搜索",
                cx,
            );
            return false;
        }

        let Some(range) = self.active_search_range(ordinal) else {
            self.set_message(
                LastOperationKind::Search,
                format!("没有第 {} 个搜索结果", ordinal + 1),
                cx,
            );
            return false;
        };
        ...
        let result = self
            .buffer
            .set_selection(SelectionSet::new(vec![Selection::new(
                range.start(),
                range.end(),
            )]));
        if result.is_err() {
            self.handle_result(result, LastOperationKind::Search, "跳转搜索结果", cx);
            return false;
        }

        if let Ok(position) = self.buffer.char_to_position(range.start()) {
            self.viewport_start_line = position.line();
            self.last_viewport_update = format!(
                "搜索跳转同步 viewport 到第 {} 行",
                self.viewport_start_line.get()
            );
        }

        self.current_search_match = Some(ordinal);
        true
    }

    fn run_literal_search(&mut self, cx: &mut Context<Self>) {
        self.search_mode = SearchMode::Literal;
        match self
            .buffer
            .search(&self.literal_query, self.literal_search_options())
        {
            Ok(result) => {
                let count = result.len();
                let ranges = result.ranges().collect::<Vec<_>>();
                self.literal_search_result = Some(result);
                self.regex_search_result = None;
                self.current_search_match = (!ranges.is_empty()).then_some(0);
                self.last_search_update = format!(
                    "literal {:?} 命中 {} 个；case_sensitive={} whole_word={}",
                    self.literal_query, count, self.search_case_sensitive, self.search_whole_word
                );
                if !self.install_search_highlights(ranges, "literal", cx) {
                    return;
                }
                if count > 0 {
                    let _ = self.select_search_match(0, cx);
                }
                self.set_message(
                    LastOperationKind::Search,
                    self.last_search_update.clone(),
                    cx,
                );
            }
            Err(error) => self.set_message(LastOperationKind::Error, format!("{error:?}"), cx),
    ...
    fn run_regex_search(&mut self, cx: &mut Context<Self>) {
        self.search_mode = SearchMode::Regex;
        match self
            .buffer
            .search_regex(&self.regex_query, self.regex_search_options())
        {
            Ok(result) => {
                let count = result.len();
                let ranges = result.ranges().collect::<Vec<_>>();
                self.regex_search_result = Some(result);
                self.literal_search_result = None;
                self.current_search_match = (!ranges.is_empty()).then_some(0);
                self.last_search_update = format!(
                    "regex {:?} 命中 {} 个；case_sensitive={} multi_line=true dot_newline=true",
                    self.regex_query, count, self.search_case_sensitive
                );
                if !self.install_search_highlights(ranges, "regex", cx) {
                    return;
                }
                if count > 0 {
                    let _ = self.select_search_match(0, cx);
    ...
    fn rerun_active_search(&mut self, cx: &mut Context<Self>) {
        match self.search_mode {
            SearchMode::Literal => self.run_literal_search(cx),
            SearchMode::Regex => self.run_regex_search(cx),
        }
    }

    fn move_search_match(&mut self, direction: MovementDirection, cx: &mut Context<Self>) {
        let len = self.active_search_len();
        if len == 0 {
            self.set_message(LastOperationKind::Search, "没有可跳转的搜索结果", cx);
            return;
        }
        if self.active_search_is_stale() {
            self.set_message(
        ...
        let current = self.current_search_match.unwrap_or(0).min(len - 1);
        let next = match direction {
            ...
            MovementDirection::Next => (current + 1) % len,
        };
        if self.select_search_match(next, cx) {
            self.set_message(
                LastOperationKind::Search,
                format!(
                    "{} 搜索结果 {}/{}",
    ...
    fn toggle_search_case(&mut self, cx: &mut Context<Self>) {
        ...
        self.last_search_update = format!(
            "搜索大小写敏感 = {}；已重新运行当前搜索",
            bool_label(self.search_case_sensitive)
        );
        self.rerun_active_search(cx);
    }
    ...
    fn toggle_search_whole_word(&mut self, cx: &mut Context<Self>) {
        ...
        self.search_mode = SearchMode::Literal;
        self.last_search_update = format!(
            "literal whole_word = {}；已重新运行普通搜索",
            bool_label(self.search_whole_word)
        );
        self.run_literal_search(cx);
    }

    fn replace_current_search_match(&mut self, cx: &mut Context<Self>) {
        let Some(ordinal) = self.current_search_match else {
            self.set_message(LastOperationKind::Search, "没有当前搜索结果可替换", cx);
            return;
        };

        let result = match self.search_mode {
            SearchMode::Literal => self.literal_search_result.clone().map(|result| {
                self.buffer
                    .replace_search_match(&result, ordinal, &self.literal_replacement)
                    .map(|_| {
                        format!(
                            "replace literal #{} 为 {:?}",
                            ordinal + 1,
                            self.literal_replacement
                        )
                    })
            }),
            SearchMode::Regex => self.regex_search_result.clone().map(|result| {
                self.buffer
                    .replace_regex_match(&result, ordinal, &self.regex_replacement)
                    .map(|_| {
                        format!(
                            "replace regex #{} 为 {:?}",
                            ordinal + 1,
        ...
        let Some(result) = result else {
            self.set_message(LastOperationKind::Search, "尚未运行搜索", cx);
            return;
        };

        match result {
            Ok(message) => {
                let message = self.append_range_sync_summary(message);
                self.last_search_update = message.clone();
                self.rerun_active_search(cx);
                self.set_message(LastOperationKind::Search, message, cx);
            }
            Err(error) => self.set_message(LastOperationKind::Error, format!("{error:?}"), cx),
        }
    }
    ...
    fn replace_all_search_matches(&mut self, cx: &mut Context<Self>) {
        let result = match self.search_mode {
            SearchMode::Literal => self.literal_search_result.clone().map(|result| {
                self.buffer
                    .replace_all_search_matches(&result, &self.literal_replacement)
                    .map(|delta| {
                        let edits = delta
                            .as_ref()
                            .map(|(delta, _)| delta.edits.len())
                            .unwrap_or(0);
                        format!(
                            "replace all literal：{} 个事务 edit，replacement={:?}",
                            edits, self.literal_replacement
                        )
                    })
            }),
            SearchMode::Regex => self.regex_search_result.clone().map(|result| {
                self.buffer
                    .replace_all_regex_matches(&result, &self.regex_replacement)
                    .map(|delta| {
                        let edits = delta
                            .as_ref()
                            .map(|(delta, _)| delta.edits.len())
                            .unwrap_or(0);
                        format!(
                            "replace all regex：{} 个事务 edit，replacement={:?}",
                            edits, self.regex_replacement
                        )
                    })
            }),
        };
    ...
    fn load_large_viewport_sample(&mut self, cx: &mut Context<Self>) {
        ...
        lines.push(
            "M12 大文本 viewport 样本：用于滚动、跳转行、长行截断和只读 slice 体感验证".to_string(),
        );
        lines.push(
            ...
            .is_some()
        {
            ...
            self.clear_search_state("加载大文本后已清空搜索结果");
            self.viewport_start_line = Line::ZERO;
    ...
    fn search_status(&self) -> String {
        ...
        let current = self
            ...
            .map(|ordinal| format!("{}/{}", ordinal + 1, len))
            .unwrap_or_else(|| "none".to_string());
        let version = match self.search_mode {
            SearchMode::Literal => self
                .literal_search_result
                .as_ref()
                .map(|result| result.version().get().to_string())
                .unwrap_or_else(|| "none".to_string()),
            SearchMode::Regex => self
                .regex_search_result
                .as_ref()
        ...
        let query = match self.search_mode {
            SearchMode::Literal => &self.literal_query,
            SearchMode::Regex => &self.regex_query,
        };
        let replacement = match self.search_mode {
            SearchMode::Literal => &self.literal_replacement,
            SearchMode::Regex => &self.regex_replacement,
        };

        format!(
            "Search：mode={} | query={:?} | replacement={:?} | matches={} | current={} | result_v={} | stale={} | case={} | whole_word={} | 最近：{}",
            ...
            current,
            version,
            bool_label(self.active_search_is_stale()),
            bool_label(self.search_case_sensitive),
            bool_label(self.search_whole_word),
            self.last_search_update,
        )
    }
    ...
    fn status_lines(&self) -> Vec<String> {
        ...
        vec![
            format!(
                "M12 Search Testbed | {} | Buffer v{} | {} | {} lines | {} chars | {} bytes",
            ...
            viewport,
            format!(
                "Search {} | matches={} | current={} | stale={}",
                self.search_mode.label(),
                self.active_search_len(),
                self.current_search_match
                    .map(|ordinal| (ordinal + 1).to_string())
                    .unwrap_or_else(|| "none".to_string()),
                self.active_search_is_stale()
            ),
    ...
    fn help_lines(&self) -> Vec<&'static str> {
        vec![
            ...
            "Search：Cmd-F literal；Cmd-Alt-F regex；Cmd-N / Cmd-Shift-N 跳转；Cmd-Shift-F 大小写；Cmd-Alt-W whole word；Cmd-H replace；Cmd-Alt-H replace all",
        ]
    }
    ...
    fn run_regex_search_action(
        &mut self,
        _: &RunRegexSearch,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        self.run_regex_search(cx);
    }

    fn search_next_action(&mut self, _: &SearchNext, _: &mut Window, cx: &mut Context<Self>) {
        self.move_search_match(MovementDirection::Next, cx);
    }
    ...
    fn search_previous_action(
        ...
        self.move_search_match(MovementDirection::Previous, cx);
    }

    fn toggle_search_case_action(
        &mut self,
        _: &ToggleSearchCase,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    fn toggle_search_whole_word_action(
        &mut self,
        _: &ToggleSearchWholeWord,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        self.toggle_search_whole_word(cx);
    }

    fn replace_current_search_match_action(
        &mut self,
        _: &ReplaceCurrentSearchMatch,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        self.replace_current_search_match(cx);
    }

    fn replace_all_search_matches_action(
        &mut self,
        _: &ReplaceAllSearchMatches,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
...
impl Focusable for M12Testbed {
    fn focus_handle(&self, _: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for M12Testbed {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .on_action(cx.listener(Self::run_literal_search_action))
            .on_action(cx.listener(Self::run_regex_search_action))
            .on_action(cx.listener(Self::search_next_action))
            .on_action(cx.listener(Self::search_previous_action))
            .on_action(cx.listener(Self::toggle_search_case_action))
            .on_action(cx.listener(Self::toggle_search_whole_word_action))
            .on_action(cx.listener(Self::replace_current_search_match_action))
            .on_action(cx.listener(Self::replace_all_search_matches_action))
            .size_full()
            .overflow_y_scroll()
...
fn initial_buffer() -> EngineResult<Buffer> {
    Buffer::from_loaded_text(
        BufferKind::file("/tmp/zom-engine-m12-testbed.txt"),
...
fn bind_keys(cx: &mut App) {
    cx.bind_keys([
        ...
        KeyBinding::new("cmd-f", RunLiteralSearch, None),
        KeyBinding::new("cmd-alt-f", RunRegexSearch, None),
        KeyBinding::new("cmd-n", SearchNext, None),
        KeyBinding::new("cmd-shift-n", SearchPrevious, None),
        KeyBinding::new("cmd-shift-f", ToggleSearchCase, None),
        KeyBinding::new("cmd-alt-w", ToggleSearchWholeWord, None),
        KeyBinding::new("cmd-h", ReplaceCurrentSearchMatch, None),
        KeyBinding::new("cmd-alt-h", ReplaceAllSearchMatches, None),
    ...
    Application::new().run(|cx: &mut App| {
        ...
        let window = cx
            .open_window(
                ...
                |_, cx| cx.new(M12Testbed::new),
            )
            .expect("open M12 testbed window");
        ...
        window
            ...
            .expect("focus M12 testbed window");
    });
}
