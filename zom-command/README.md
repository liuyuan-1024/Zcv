...
键盘快捷键、命令面板和菜单等入口都可以把可命名动作收敛成 `(CommandId, CommandArgs)`，经唯一命令派发路径进入执行器。连续输入设备事件由桌面端输入意图层处理。

它是命令的汇聚点（sink），不是被各层 import 的服务：输入源产出意图，由组合根喂给执行器。

它依赖「它要编辑的东西」（`workspace` / `view` / `engine`）。内建编辑命令是本 crate 存在的理由，放在这里并保持可无头测试；需要宿主资源的命令由 `zom-desktop` 组合根接线。

## 核心能力

- **命令注册表**：`CommandRegistry` 存放当前内建命令的元数据与 handler。
- **具体的 `CommandContext`**：持有 `&mut Workspace` / `&mut ViewSet` / `&mut CommandQueue` / `&mut EffectQueue`，暴露整个 workspace 与 view-set；需要宿主接力的动作通过 `HostEffect` 发出。
- **命令队列组合**：`CommandQueue` 让 handler 入队子命令，执行器排空，不重入；宏就是记录命令队列。
