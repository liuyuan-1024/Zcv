...
//!
//! 自持小输入框由 [`OwnedEditorTarget`] 持有一份引擎 `Buffer` + 选区。
//! 编辑命令通过 [`zom_command::CommandContext::focused_field`] 路由到聚焦目标，从而共享 `zom-command` 的全部编辑能力。
...
pub(crate) use target::OwnedEditorTarget;
