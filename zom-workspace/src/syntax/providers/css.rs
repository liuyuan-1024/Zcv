//! tree-sitter-css Tier 1 provider。
//!
...
pub(crate) fn css_config() -> Result<Arc<SharedConfig>, &'static QueryError> {
...
pub fn new_provider() -> HighlightWorker {
    let config = css_config().expect("tree-sitter-css 高亮配置必须构建");
    HighlightWorker::new(LanguageId::new("css"), config)
}
...
mod tests {
    ...
    const SAMPLE: &str = "body { color: #fff; background: rgb(0, 0, 0); }\n";

    #[test]
    fn css_provider_emits_spans() {
        smoke_test_provider(LanguageId::new("css"), SAMPLE, new_provider);
    }

    #[test]
    fn lookup_table_matches_query_capture_names() {
        let cfg = css_config().expect("css 配置必须构建");
