...
fn invalid_utf8_byte_boundary_should_be_rejected_by_byte_projection_and_slice() {
    ...
    assert!(matches!(
        coordinate,
        TextError::Coordinate(CoordinateError::InvalidByteBoundary(offset)) if offset == b(1)
    ));
    assert!(matches!(
        slice,
        TextError::Coordinate(CoordinateError::InvalidByteBoundary(offset)) if offset == b(1)
...
fn crlf_middle_should_not_be_valid_line_position_or_edit_boundary() {
    ...
    assert!(matches!(
        char_err,
        TextError::Coordinate(CoordinateError::CharOutOfBounds(offset)) if offset == c(2)
    ));
    assert!(matches!(
        byte_err,
...
fn grapheme_boundary_should_reject_combining_mark_middle_and_map_to_nearest_boundaries() {
    ...
    assert!(matches!(
        ...
        TextError::Coordinate(CoordinateError::InvalidGraphemeBoundary(offset)) if offset == b(2)
    ));
