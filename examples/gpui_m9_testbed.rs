...
impl M9Testbed {
        ...
        match self.buffer.undo() {
            Ok(Some(_)) => {
                let tracked = self
                    ...
                    .unwrap_or_default();
                self.set_message(LastOperationKind::History, format!("已 undo{tracked}"), cx);
            }
        ...
        match self.buffer.redo() {
            Ok(Some(_)) => {
                let tracked = self
                    ...
                    .map(|update| format!("；{update}"))
                    .unwrap_or_default();
                self.set_message(LastOperationKind::History, format!("已 redo{tracked}"), cx);
            }
    ...
    fn status_lines(&self) -> Vec<String> {
        ...
        vec![
            format!(
                "M9 GPUI testbed | {} | v{} | 保存点={} | 行数={} chars={} bytes={}",
    ...
    fn help_lines(&self) -> Vec<&'static str> {
        vec![
            ...
            "M9 事件：状态栏显示最近 DeltaEvent / PositionMap；Cmd-Shift-E 清空 pending DeltaEvent 队列",
            "M9 tracked range：Cmd-T 从 primary selection 创建；Cmd-Shift-T 创建 demo ranges；Cmd-Alt-T 清空；输入 / 删除 / Undo / Redo 后观察移动、收缩、失效",
...
fn decorate_text(
    ...
    out.push_str("图例：┃ caret，⟦selection⟧ range，〖composition preedit〗 range，‹tracked› range，◆ tracked caret。所有 offset 都是 CharOffset。\n\n");
