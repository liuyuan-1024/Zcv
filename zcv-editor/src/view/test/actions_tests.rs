...
fn editors_share_buffer_but_keep_view_state_independent(cx: &mut TestAppContext) {
    ...
    cx.update_entity(&first, |editor, cx| {
        ...
        editor.multi_buffer().update(cx, |multi_buffer, cx| {
            multi_buffer
                .edit(
                    vec![Edit::insert(MultiBufferOffset::new(3).into(), "d").unwrap()],
...
fn document_boundary_actions_move_and_extend_selection(cx: &mut TestAppContext) {
    ...
    let end = MultiBufferOffset::new(text.len());
...
fn page_actions_move_selection_and_viewport_together(cx: &mut TestAppContext) {
    ...
    let snapshot = cx.read_entity(&buffer, |buffer, _| buffer.text_snapshot());
    let first_page: MultiBufferOffset = snapshot
        .line_start_byte(Line::new(page_rows))
        .expect("第一页目标行应存在")
        .into();
    let second_page: MultiBufferOffset = snapshot
...
fn composite_excerpt_uses_its_source_tree_sitter_indent_query(cx: &mut TestAppContext) {
    ...
    let raw_buffer = Buffer::from_text(source_text.to_owned(), BufferConfig::default())
        .expect("Rust 测试 Buffer 应能创建");
    let language_buffer = cx.new({
        move |cx| {
            LanguageBuffer::new(
    ...
    cx.update_entity(&combined, |buffer, cx| {
        buffer.set_excerpts(
            vec![ExcerptRange::new(
                ...
                MultiBufferRange::new(
                    MultiBufferOffset::ZERO,
                    MultiBufferOffset::new(source_text.len()),
                )
                .unwrap()
                .into(),
                Vec::new(),
