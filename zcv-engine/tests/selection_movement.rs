...
#[test]
fn selection_edit_should_reject_out_of_bounds_invalid_utf8_and_grapheme_middle_offsets_atomically()
{
    for (text, offset) in [("abc", 4), ("你a", 1), ("ae\u{301}b", 2), ("a\r\nb", 2)] {
        let mut buffer = buffer(text);
        let original_text = buffer_text(&buffer);
        let original_version = buffer.version();

        assert!(
            buffer
                .insert_at_selections(&set_caret(offset), "X", metadata("insert"))
                .is_err()
        );
        assert_eq!(buffer_text(&buffer), original_text);
...
fn multi_cursor_insert_replace_delete_should_apply_one_state_transition_per_command() {
    ...
    assert_eq!(
        outcome.after_selections().ranges(),
        vec![range(2, 2), range(6, 6)]
    );
    ...
    assert_eq!(
        outcome.after_selections().ranges(),
        vec![range(1, 1), range(3, 3)]
    );
}

#[test]
fn one_buffer_should_accept_independent_selection_sets_from_two_editor_views() {
    let mut buffer = buffer("abcd");
    let editor_a = set_caret(1);
    let editor_b = set_caret(0);

    let editor_a_outcome = buffer
        .insert_at_selections(&editor_a, "X", metadata("insert"))
        .unwrap();
    let editor_b_outcome = buffer
        .insert_at_selections(&editor_b, "Y", metadata("insert"))
        .unwrap();

    assert_eq!(buffer_text(&buffer), "YaXbcd");
    assert_eq!(
        editor_a_outcome.after_selections().ranges(),
        vec![range(2, 2)]
    );
    assert_eq!(
        editor_b_outcome.after_selections().ranges(),
        vec![range(1, 1)]
    );
}
...
fn delete_at_selections_should_respect_grapheme_clusters() {
    ...
    assert_eq!(outcome.after_selections().ranges(), vec![range(1, 1)]);
...
#[test]
fn ordinary_transaction_should_leave_selection_mapping_to_the_caller() {
    let mut buffer = buffer("abcd");
    let selections = SelectionSet::new(vec![selection(4, 2)]);

    let transaction = tx(&buffer, vec![Edit::insert(b(1), "X".to_string()).unwrap()]);
    let (_, changeset) = buffer.apply_transaction(transaction).unwrap();
    let mapped = changeset.position_map().map_selection_set(&selections);

    let mapped = mapped.primary();
...
fn backward_word_move_treats_newline_as_separator_category() {
    ...
    let moved = buffer
        .move_selections(
            &set_caret(empty_line_start.get()),
...
fn backward_word_delete_from_empty_line_start_should_remove_empty_line_separator() {
    ...
    assert_eq!(buffer_text(&buffer), "abc\nnext");
    assert_eq!(result.after_selections().primary().head(), b("abc".len()));
}
...
#[test]
fn move_selections_should_collapse_or_extend_from_existing_anchor() {
...
fn indent_at_caret_should_insert_soft_tab_aligned_to_next_indent_stop() {
    ...
    let outcome = buffer.indent_at_selections(&set_caret(2)).unwrap();
    assert_eq!(buffer_text(&buffer), "ab  c");
    assert_eq!(outcome.after_selections().primary().head(), b(4));
}
...
fn indent_at_caret_on_first_column_should_insert_full_indent_width() {
    ...
    let outcome = buffer.indent_at_selections(&set_caret(0)).unwrap();
    assert_eq!(buffer_text(&buffer), "    abc");
    assert_eq!(outcome.after_selections().primary().head(), b(4));
}
...
fn indent_at_caret_after_existing_tab_should_account_for_display_column() {
    ...
    let mut buffer = buffer("\tabc");
    let outcome = buffer.indent_at_selections(&set_caret(1)).unwrap();
    assert_eq!(buffer_text(&buffer), "\t    abc");
    assert_eq!(outcome.after_selections().primary().head(), b(5));
}
...
fn indent_with_insert_spaces_false_should_insert_literal_tab_character() {
    ...
    assert_eq!(outcome.after_selections().primary().head(), b(2));
}
