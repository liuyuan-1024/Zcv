...
impl ChangeSet {
    ...
    pub fn changed_ranges(&self) -> TextResult<Vec<TextRange>> {
        ...
        for edit in self.edits.as_slice() {
            ...
            let new_start = shift.apply_old_to_new(range.start()).ok_or_else(|| {
                TextError::InvariantViolation {
                    location: "ChangeSet::changed_ranges",
                    detail: "changed range 起点在字节偏移映射时溢出".to_string(),
                }
            })?;
            let new_end = new_start.checked_add(replacement_len).ok_or_else(|| {
                TextError::InvariantViolation {
                    location: "ChangeSet::changed_ranges",
...
fn text_range(start: ByteOffset, end: ByteOffset, location: &'static str) -> TextResult<TextRange> {
