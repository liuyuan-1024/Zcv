...
pub(crate) struct TreeDrag {
    /// 被拖行路径。
    pub(crate) active_selection: AbsolutePathBuf,
    /// 渲染期快照的多选标记集（可见行序）。
    pub(crate) marked_selections: Rc<[AbsolutePathBuf]>,
    /// 跟随预览显示的条目名（被拖行的名称）。
...
impl TreeDrag {
    ...
    pub(crate) fn items(&self) -> Vec<AbsolutePathBuf> {
...
pub(crate) fn filter_movable_sources(
