...
use zcv_engine::{
    ...
    TrackedRangeUpdatePolicy, VisibleLine,
};
...
impl FoldSnapshot {
    ...
    pub(super) fn project_text_range(
        ...
        let start = LogicalPoint::from(self.buffer_snapshot.byte_to_position(range.start())?);
        let end = LogicalPoint::from(self.buffer_snapshot.byte_to_position(range.end())?);
        let logical = LogicalRange::new(start, end)?;
        if logical.is_empty() {
            return Ok(Vec::new());
        }
        let start = match self.logical_to_projected_point(logical.start())? {
            LogicalPointProjection::Visible(point) => point,
            LogicalPointProjection::Hidden {
                anchor_projected, ..
            } => anchor_projected,
        };
        let end = match self.logical_to_projected_point(logical.end())? {
            LogicalPointProjection::Visible(point) => point,
            LogicalPointProjection::Hidden {
                anchor_projected, ..
            } => ProjectedPoint::line_start(ProjectedLineIndex::new(
        ...
        let mut previous_is_placeholder = self
            .projected_line_kind(start.line())
            .is_some_and(|kind| kind.is_placeholder());
        for row in (start.line().get() + 1)..=end.line().get() {
            let line = ProjectedLineIndex::new(row);
            let is_placeholder = self
                .projected_line_kind(line)
                .is_some_and(|kind| kind.is_placeholder());
            if is_placeholder != previous_is_placeholder {
                breakpoints.push(ProjectedPoint::line_start(line));
        ...
        breakpoints
            ...
            .map(|window| ProjectedRange::new(window[0], window[1]).map_err(Into::into))
            .collect()
    }
    ...
    pub(super) fn slice_viewport(
        ...
        for value in start..end {
            let index = ProjectedLineIndex::new(value);
            match self
                .projected_line_kind(index)
                .expect("clamp 后投影行必须存在")
            {
                ProjectedLineKind::Text(text) => rows.push(ProjectedViewportRow::new(
                    index,
                    ProjectedViewportRowKind::Text {
                        logical_line: text.logical_line(),
                        visible: self
                            .buffer_snapshot
                            .visible_line(text.logical_line(), viewport.max_line_chars())?,
                    },
                )),
...
mod tests {
    ...
    #[test]
    fn projected_text_range_splits_at_placeholder_boundaries() {
        let buffer = Buffer::scratch("a\nb\nc\nd".to_string(), BufferConfig::default()).unwrap();
        let (mut map, _) = FoldMap::new(buffer.snapshot());
        let (snapshot, _) = map.write().fold_lines(line_range(0, 3)).unwrap();

        let segments = snapshot
            .project_text_range(text_range(0, buffer.snapshot().len_bytes().get()))
            .unwrap();

        assert_eq!(segments.len(), 3);
        assert_eq!(segments[0].start().line(), ProjectedLineIndex::ZERO);
        assert_eq!(segments[1].start().line(), ProjectedLineIndex::new(1));
        assert_eq!(segments[2].start().line(), ProjectedLineIndex::new(2));
    }
}
...
impl<'a> ProjectedViewportRow<'a> {
    pub(crate) fn new(index: ProjectedLineIndex, kind: ProjectedViewportRowKind<'a>) -> Self {
        Self { index, kind }
    }
...
impl ProjectedLineRange {
    pub(crate) fn new(start: ProjectedLineIndex, end: ProjectedLineIndex) -> Self {
...
fn build_logical_spans<'a>(rows: &[ProjectedViewportRow<'a>]) -> DisplayMapResult<Vec<LineRange>> {
