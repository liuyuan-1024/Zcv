...
fn translocated_paths_are_detected_by_component() {
    assert!(
        UpdateInstallation::from_process_path(Path::new(
            "/private/var/folders/ab/cd/T/AppTranslocation/uuid/d/Zcv.app"
        ))
        .is_err()
    );
    assert!(UpdateInstallation::from_process_path(Path::new("/Applications/Zcv.app")).is_ok());
    assert!(UpdateInstallation::from_process_path(Path::new("/tmp/Zcv.app")).is_ok());
}
