//! M10 GPUI testbed：MetadataLayer、TrackedRange、PositionMap、DeltaEvent 与 M8 既有编辑体验。
//!
//! 这个 example 是“人类体感 / UI 桥接”验证入口，不替代 `tests/m10_metadata_layer.rs`。
//! 它继承 M8 的输入、移动、多光标、Undo / Redo、composition、reload、保存边界、
//! DeltaEvent、PositionMap 与 M9 TrackedRange 体感，并叠加 MetadataLayer 创建、跟随、
//! 查询、替换与过期丢弃观察。
...
const SAMPLE_TEXT: &str = "M10 MetadataLayer / PositionMap / DeltaEvent 体验台\n\n\
英文区域：hello world\n\
中文输入区域：|\n\
日文输入区域：|\n\
韩文输入区域：|\n\
...
\n\
M10 状态观察：BufferId / BufferState / DeltaEvent / PositionMap / TrackedRange / MetadataLayer。\n\
试试输入、删除、替换、Undo/Redo、composition、reload，并观察 tracked range 与 metadata range 移动、查询、替换和过期丢弃。";

const RELOAD_TEXT: &str = "M10 reload 后的新外部文本\n\n\
...
impl M10Testbed {
    fn new(cx: &mut Context<Self>) -> Self {
        let mut buffer = initial_buffer().expect("M10 testbed sample text should be valid");
        buffer.mark_saved();
        ...
        let mut this = Self {
            ...
            message: "M10 已就绪：PositionMap / DeltaEvent / pending events".to_string(),
    ...
    fn handle_result<T>(
        ...
        match result {
            Ok(value) => {
                let mut message = ok_message.into();
    ...
    fn undo(&mut self, cx: &mut Context<Self>) {
        match self.buffer.undo() {
            Ok(Some(_)) => {
                let tracked = self
                    .sync_tracked_ranges_from_last_event()
                    .map(|update| format!("；{update}"))
                    .unwrap_or_default();
                let metadata = self
                    .sync_metadata_layers_from_last_event()
                    .map(|update| format!("；{update}"))
                    .unwrap_or_default();
                self.set_message(
                    LastOperationKind::History,
                    format!("已 undo{tracked}{metadata}"),
                    cx,
                );
            }
    ...
    fn redo(&mut self, cx: &mut Context<Self>) {
        match self.buffer.redo() {
            Ok(Some(_)) => {
                ...
                self.set_message(
                    LastOperationKind::History,
                    format!("已 redo{tracked}{metadata}"),
                    cx,
                );
            }
    ...
    fn buffer_lifecycle_status(&self) -> String {
        ...
        format!(
            "Buffer：id={} | kind={} | state={:?} | read_only={} | dirty={} | 可直接关闭={} | saved_version={} | last_saved_version={} | external_sync={}",
            self.buffer.id().get(),
    ...
    fn loaded_text_status(&self) -> String {
        self.buffer
            ...
            .map(|info| {
                format!(
                    "LoadedText：encoding={:?} | BOM={:?}/{} | invalid UTF-8={:?}/{} | line ending={:?} | final newline={}",
                    info.encoding,
                    info.bom_policy,
                    info.had_bom,
            ...
            .unwrap_or_else(|| {
                "LoadedText：none（当前文本不是 from_loaded_text 创建，或已 reload）".to_string()
            })
    }
    ...
    fn status_lines(&self) -> Vec<String> {
        ...
        vec![
            format!(
                "M10 GPUI testbed | {} | v{} | 保存点={} | 行数={} chars={} bytes={}",
    ...
    fn help_lines(&self) -> Vec<&'static str> {
        vec![
            ...
            "composition：Cmd-I start，Cmd-K update raw 'n'，Cmd-L update '你'，Cmd-Y update '你好'",
...
impl Focusable for M10Testbed {
    fn focus_handle(&self, _: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for M10Testbed {
...
fn initial_buffer() -> EngineResult<Buffer> {
    Buffer::from_loaded_text(
        BufferKind::file("/tmp/zom-engine-m10-testbed.txt"),
...
fn metadata_kind_label(kind: &MetadataLayerKind) -> String {
    match kind {
        MetadataLayerKind::SearchMatch => "SearchMatch".to_string(),
        MetadataLayerKind::Diagnostics => "Diagnostics".to_string(),
        MetadataLayerKind::SyntaxHighlight => "SyntaxHighlight".to_string(),
        MetadataLayerKind::SemanticToken => "SemanticToken".to_string(),
        MetadataLayerKind::Breakpoint => "Breakpoint".to_string(),
        MetadataLayerKind::Bookmark => "Bookmark".to_string(),
        MetadataLayerKind::InlayHint => "InlayHint".to_string(),
...
fn decorate_text(
    ...
    out.push_str("图例：┃ caret，⟦selection⟧ range，〖composition preedit〗 range，‹tracked› range，◆ tracked caret，〔metadata〕 range，◇ metadata caret。所有 offset 都是 CharOffset。\n\n");
    ...
    Application::new().run(|cx: &mut App| {
        ...
        let window = cx
            .open_window(
                ...
                |_, cx| cx.new(M10Testbed::new),
            )
            .expect("open M10 testbed window");
        ...
        window
            ...
            .expect("focus M10 testbed window");
    });
}
