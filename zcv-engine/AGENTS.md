...
> AI 工具参与 `zcv-engine` 前先读本文档。本文只写稳定规则；当前状态、结构概览、建议命令看 `docs/STATUS.md`。
...
`zcv-engine` 是独立 Rust 纯文本编辑引擎 crate，为编辑器宿主提供底层文本编辑能力。
...
只是“宿主可能用得到”的能力，不默认进入 `zcv-engine`。
