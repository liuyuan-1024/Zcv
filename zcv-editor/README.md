...
`MultiBufferAnchor` 绑定源版本，经不衰减坐标索引解析。选择和滚动通过组合层的总坐标解析消费它：diff 或 excerpt 退出投影时落到当前结构边界，空投影落到文首；源版本链无法推进则显式失败。折叠、搜索等必须附着源内容的状态使用仅限当前投影的解析，在源退出时由其所有者失效，不与选择共用补偿逻辑。
